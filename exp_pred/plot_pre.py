...
# Load predicted data
paths = {
    'Real Data': 'exp_pred/pred_results/LGB_read_data_pred_results.pickle',
    'GMMs': 'exp_pred/pred_results/LGB_gmm_pred_results_1.0.pickle',
    'FCPflow': 'exp_pred/pred_results/LGB_flow_pred_results_1.0.pickle',
    't-Copula': 'exp_pred/pred_results/LGB_copula_pred_results_1.0.pickle',
    'DoppelGANger': 'exp_pred/pred_results/LGB_DoppelGANger_pred_results_1.0.pickle'
}

with open(paths['Real Data'], 'rb') as f:
    real_data = pickle.load(f)
with open(paths['GMMs'], 'rb') as f:
    gmm_data = pickle.load(f)
with open(paths['FCPflow'], 'rb') as f:
    flow_data = pickle.load(f)
with open(paths['t-Copula'], 'rb') as f:
    copula_data = pickle.load(f)
with open(paths['DoppelGANger'], 'rb') as f:
    doppel_data = pickle.load(f)
...
# Define datasets
datasets = {
    'Target': real_data_test,
    'Real Data': real_data,
    'GMMs': gmm_data,
    'FCPflow': flow_data,
    't-Copula': copula_data,
    'DoppelGANger': doppel_data
}

# Define colors and linestyles
colors = ['b', 'orange', 'r', 'k', 'm', 'g']
linestyles_list = ['-', '--', ':', '-.', '--', ':']
linestyles = {key: value for key, value in zip(datasets.keys(), linestyles_list)}
color_map = {key: value for key, value in zip(datasets.keys(), colors)}

# Plot
index = 3
plt.figure(figsize=(10, 4))
for name, data in datasets.items():
    plt.plot(data[index].T, label=name, color=color_map[name], linestyle=linestyles[name])
