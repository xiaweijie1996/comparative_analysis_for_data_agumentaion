...
def feature_lag(full_input_data, 
                full_output_data,
                lag=config["Data"]["lag"],
                lag_features=config["Data"]["lag_features"]):
    
    # Create a copy of the input data
    input_data = full_input_data.copy()

    # Find the lag features
    lag_frame = input_data[lag_features]
    
    # Shift the lag features based on the lag
    for i in lag:
        _lag_frame = lag_frame.shift(-i)
        input_data = pd.concat([input_data, _lag_frame.add_suffix(f'_lag_{i}')], axis=1)
    
    # Get the index of the rows with nan values
    nan_index = input_data.isnull().any(axis=1)
    
    # Drop the rows of nan index in input and output data
    input_data = input_data[~nan_index]
    output_data = full_output_data[~nan_index]
...
if __name__ == "__main__":
    ...
    print(full_input_data.shape)
    
    lag_input_data, lag_output_data = feature_lag(full_input_data, full_output_data)
    print(lag_input_data.shape, lag_output_data.shape)
    print('--------------------------------------')
